...
export function SourceAssetCard({
  ...
  return (
    ...
    >
      {layout === "stack" && topContent ? <div className="mb-3">{topContent}</div> : null}
      <div className={cn(layout === "rail" ? "flex gap-3" : "block")}>
        <div
          className={cn(
            "relative",
            layout === "rail" ? "w-24 shrink-0 self-start" : undefined,
          )}
        >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            <div
              className="mt-0.5 h-5 w-5 flex-shrink-0 rounded-full border border-border"
              style={{ background: asset.averageColor }}
            />
          </div>

          {layout === "rail" && topContent ? (
            <div className={cn(layout === "rail" ? "mt-3" : "mb-3 mt-0")}>
              {topContent}
            </div>
          ) : null}
        </div>
